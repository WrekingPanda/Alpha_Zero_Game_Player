...
class AlphaZero:
    ...
    def SelfPlay(self, verbose=False, size = 0):
        if self.fill_size==0:
        ...
        if self.fill_size==0:
            ...
            self.mcts = MCTS(board, self.params["mcts_iterations"], self.model, fill_size=self.fill_size)
            ...
            if board.hasFinished():
                if verbose:
                    with open(f"./selfplay_{self.gameType}{board.size}.txt", 'a') as file:
                        ...
                        file.close()
                return_dataset = [(board.EncodedGameStateChanged(self.fill_size), action_probs, board.winner)]
    ...
    def Learn(self):
        for iteration in tqdm(range(self.params["n_iterations"]), desc="AlphaZero Algorithm Iterations", leave=False, unit="iter", ncols=100, colour="#fc6a65"):
            ...
            for sp_iteration in tqdm(range(self.params["self_play_iterations"]), desc="Self-Play Iterations", leave=False, unit="iter", ncols=100, colour="#fca965"):
                if self.fill_size!=0:
                    if sp_iteration%(self.params["self_play_iterations"]/(self.fill_size-3)) == 0:
                        board_size-=1
                    dataset += self.SelfPlay(verbose=self.verbose, size = board_size)
                else:
